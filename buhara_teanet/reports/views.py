...
def worker_period_report_pdf_view(request, period):
    today = now().date()
    ...
    if period == 'daily':
        ...
        messages.error(request, "Invalid report period selected.")
    ...
    pdf.drawString(50, y, f"Generated for: {request.user.username}")
    y -= 15
    pdf.drawString(50, y, f"Generated on: {now().strftime('%Y-%m-%d %H:%M')}")
    y -= 15
    pdf.drawString(50, y, f"Period starting: {start_date}")
    y -= 30
    ...
    pdf.setFont("Helvetica-Bold", 12)
    pdf.drawString(50, y, "Submission Records")
    y -= 20

    pdf.setFont("Helvetica-Bold", 9)
    pdf.drawString(50, y, "Case ID")
    ...
    for sub in submissions:
        ...
        pdf.drawString(50, y, f"CASE-{sub.id:04d}")
        pdf.drawString(130, y, sub.get_status_display())
        pdf.drawString(250, y, str(sub.location or "Not specified")[:25])
        pdf.drawString(420, y, sub.created_at.strftime('%Y-%m-%d'))
        y -= 15

    if not submissions:
        pdf.drawString(50, y, "No submissions found for this period.")
    ...
    filename = f"{period}_submission_report_{request.user.username}.pdf"
